...
| `build:dev`   | Development with cold reloading: start app using npm package nodemon and ts-node on root file ./src/index.ts, watching for changes to .ts and .js and .json files from within watch=*.* (by default as not set).
nodemon --exec ts-node --ext ts,js,json -- ./src/index.ts |
